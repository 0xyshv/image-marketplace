import { CacheableContractInteractionsLoader, CacheableExecutorFactory, CacheableStateEvaluator, Evolve } from '../../plugins/index';
import { ContractDefinitionLoader, ContractInteractionsLoader, DefaultStateEvaluator, HandlerExecutorFactory, LexicographicalInteractionsSorter, SmartWeave } from '../index';
...
export class SmartWeaveWebFactory {
    ...
    static remoteCacheBased(arweave, cacheBaseURL) {
        ...
        const interactionsLoader = new CacheableContractInteractionsLoader(new ContractInteractionsLoader(arweave), new RemoteBlockHeightCache('INTERACTIONS', cacheBaseURL));
    ...
    static memCachedBased(arweave, maxStoredBlockHeights = Number.MAX_SAFE_INTEGER) {
        const definitionLoader = new ContractDefinitionLoader(arweave, new MemCache());
        const interactionsLoader = new CacheableContractInteractionsLoader(new ContractInteractionsLoader(arweave), new MemBlockHeightSwCache(maxStoredBlockHeights));
        const executorFactory = new CacheableExecutorFactory(arweave, new HandlerExecutorFactory(arweave), new MemCache());
