...
class SmartWeaveWebFactory {
    ...
    static remoteCacheBased(arweave, cacheBaseURL) {
        ...
        const interactionsLoader = new plugins_1.CacheableContractInteractionsLoader(new core_1.ContractInteractionsLoader(arweave), new cache_1.RemoteBlockHeightCache('INTERACTIONS', cacheBaseURL));
        ...
        const stateEvaluator = new plugins_1.CacheableStateEvaluator(arweave, new cache_1.RemoteBlockHeightCache('STATE', cacheBaseURL), [new plugins_1.Evolve(definitionLoader, executorFactory)]);
     ...
     */
    static memCached(arweave, maxStoredBlockHeights = Number.MAX_SAFE_INTEGER) {
     ...
     * Use {@link SmartWeaveBuilder.build()} to finish the configuration.
     */
    static memCachedBased(arweave, maxStoredBlockHeights = Number.MAX_SAFE_INTEGER) {
        const definitionLoader = new core_1.ContractDefinitionLoader(arweave, new cache_1.MemCache());
        const interactionsLoader = new plugins_1.CacheableContractInteractionsLoader(new core_1.ContractInteractionsLoader(arweave), new cache_1.MemBlockHeightSwCache(maxStoredBlockHeights));
        const executorFactory = new plugins_1.CacheableExecutorFactory(arweave, new core_1.HandlerExecutorFactory(arweave), new cache_1.MemCache());
        const stateEvaluator = new plugins_1.CacheableStateEvaluator(arweave, new cache_1.MemBlockHeightSwCache(maxStoredBlockHeights), [new plugins_1.Evolve(definitionLoader, executorFactory)]);
        const interactionsSorter = new core_1.LexicographicalInteractionsSorter(arweave);
        return core_1.SmartWeave.builder(arweave)
            .setDefinitionLoader(definitionLoader)
            .setInteractionsLoader(interactionsLoader)
